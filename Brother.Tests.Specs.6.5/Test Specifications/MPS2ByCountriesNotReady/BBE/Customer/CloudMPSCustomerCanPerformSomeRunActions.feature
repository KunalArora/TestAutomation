@ignore @TEST @UAT @MPS
Feature: BelgianCustomerCanPerformSomeRunActions
	In order for customer to perform some run portion actions
	As a customer
	I want to be to sign in using my newly generated credentials


Scenario Outline: Belgian Customer cannot order consumable before initial communication with device
...
Scenario Outline: Belgian Customer can order consumable after initial communication with device
