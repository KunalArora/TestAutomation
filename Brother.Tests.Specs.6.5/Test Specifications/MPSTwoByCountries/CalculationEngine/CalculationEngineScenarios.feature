...
Scenarios:
	...
	| Cloud MPS Local Office Approver | Sweden         | Purchase & click inklusive service | Minimum volym                             | Cloud MPS Dealer | Email  | 36 månader | Kvartalsvis i efterskott    | Pay upfront | Brother      | Yes      | MFC-L8650CDW | HL-L5100DN  | A1T010037    | A1T010038     | A1T010039     | A1T010040     | 1000 | 1000   |
...
Scenario Outline: Scenario One Dutch
	Given "<Country>" Dealer "<Language>" have created "<ContractType>" contract with "<UsageType>" and "<Length>" and "<Billing>" and "<ServicePack>" and "<Installation>" and "<Delivery>" and "<Device1>" and "<Device2>"
	And I sign into Cloud MPS as a "<Role>" from "<Country>"
	And the contract created above is approved
	And I sign back into Cloud MPS as a "<Role1>" from "<Country>"
	#And I generate installation request for the contract with "<Method>" communication and "<Type>" installation
...
Scenarios:
	...
	| Role                            | Country | ContractType                  | UsageType      | Role1            | Method | Length | Billing              | ServicePack | Installation | Delivery | Device1      | Device2     | SerialNumber | SerialNumber1 | SerialNumber2 | SerialNumber3 | Mono | Colour | Language |
	| Cloud MPS Local Office Approver | Belgium | Purchase & Click with Service | Minimum Volume | Cloud MPS Dealer | Email  | 3 jaar | Quarterly in Arrears | Pay upfront | Brother      | Yes      | MFC-L8650CDW | MFC-L5750DW | A1T010061    | A1T010062     | A1T010063     | A1T010064     | 1000 | 1000   | Dutch    |
...
Scenarios:
	...
	| Cloud MPS Local Office Approver | Sweden         | Purchase & click inklusive service | Minimum volym                             | Cloud MPS Dealer | Email  | 48 månader | Halvårsvis   | Included in Click Price | Brother      | No       | MFC-L8650CDW | HL-L5100DN  | A1T010093    | A1T010094     | A1T010095     | A1T010096     | 10000 | 10000  |
...
Scenario Outline: Scenario Two Dutch
	Given "<Country>" Dealer "<Language>" have created "<ContractType>" contract with "<UsageType>" and "<Length>" and "<Billing>" and "<ServicePack>" and "<Installation>" and "<Delivery>" and "<Device1>" and "<Device2>"
	And I sign into Cloud MPS as a "<Role>" from "<Country>"
	And the contract created above is approved
	And I sign back into Cloud MPS as a "<Role1>" from "<Country>"
...
Scenarios:
	...
	| Role                            | Country | ContractType                  | UsageType      | Role1            | Method | Length | Billing     | ServicePack             | Installation | Delivery | Device1      | Device2     | SerialNumber | SerialNumber1 | SerialNumber2 | SerialNumber3 | Mono  | Colour | Language |
	| Cloud MPS Local Office Approver | Belgium | Purchase & Click with Service | Minimum Volume | Cloud MPS Dealer | Email  | 4 jaar | Half Yearly | Included in Click Price | Brother      | No       | MFC-L8650CDW | MFC-L5750DW | A1T010121    | A1T010122     | A1T010123     | A1T010124     | 10000 | 10000  | Dutch    |
...
Scenarios:
	...
	| Cloud MPS Local Office Approver | Sweden         | Purchase & click inklusive service | Minimum volym                             | Cloud MPS Dealer | Email  | 60 månader | Månadsvis            | Included in Click Price | Brother      | Yes      | MFC-L8650CDW | HL-L5100DN  | A1T010153    | A1T010154     | A1T010155     | A1T010156     | 5000 | 5000   |
...
Scenario Outline: Scenario Three Dutch
	Given "<Country>" Dealer "<Language>" have created "<ContractType>" contract with "<UsageType>" and "<Length>" and "<Billing>" and "<ServicePack>" and "<Installation>" and "<Delivery>" and "<Device1>" and "<Device2>"
	And I sign into Cloud MPS as a "<Role>" from "<Country>"
	And the contract created above is approved
	And I sign back into Cloud MPS as a "<Role1>" from "<Country>"
...
Scenarios:
	...
	| Role                            | Country | ContractType                  | UsageType      | Role1            | Method | Length | Billing | ServicePack             | Installation | Delivery | Device1      | Device2     | SerialNumber | SerialNumber1 | SerialNumber2 | SerialNumber3 | Mono | Colour | Language |
	| Cloud MPS Local Office Approver | Belgium | Purchase & Click with Service | Minimum Volume | Cloud MPS Dealer | Email  | 5 jaar | Monthly | Included in Click Price | Brother      | Yes      | MFC-L8650CDW | MFC-L5750DW | A1T010181    | A1T010182     | A1T010183     | A1T010184     | 5000 | 5000   | Dutch    |
...
Scenarios:
	...
	| Cloud MPS Local Office Approver | Poland         | Buy + Click                        | Bez pakietu wydruków                           | Cloud MPS Dealer | Email  | 3 lata     | Kwartalny/Quarterly in Arrears | Pay upfront | Brother      | Yes      | MFC-L8650CDW | MFC-L5750DW | A1T010229    | A1T010230     | A1T010231     | A1T010232     | 5000 | 5000   |
...
Scenario Outline: Scenario Four Dutch
	Given "<Country>" Dealer "<Language>" have created "<ContractType>" contract with "<UsageType>" and "<Length>" and "<Billing>" and "<ServicePack>" and "<Installation>" and "<Delivery>" and "<Device1>" and "<Device2>"
	And I sign into Cloud MPS as a "<Role>" from "<Country>"
	And the contract created above is approved
	And I sign back into Cloud MPS as a "<Role1>" from "<Country>"
...
Scenarios:
	| Role                            | Country | ContractType                  | UsageType          | Role1            | Method | Length | Billing              | ServicePack             | Installation | Delivery | Device1      | Device2     | SerialNumber | SerialNumber1 | SerialNumber2 | SerialNumber3 | Mono | Colour | Language |
	| Cloud MPS Local Office Approver | Belgium | Purchase & Click with Service | Werkelijk verbruik | Cloud MPS Dealer | Email  | 3 jaar | Quarterly in Arrears | Included in Click Price | Brother      | Yes      | MFC-L8650CDW | MFC-L5750DW | A1T010241    | A1T010242     | A1T010243     | A1T010244     | 5000 | 5000   | Dutch    |
