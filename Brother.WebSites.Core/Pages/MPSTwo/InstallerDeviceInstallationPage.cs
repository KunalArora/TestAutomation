...
namespace Brother.WebSites.Core.Pages.MPSTwo
{
    public class InstallerDeviceInstallationPage : BasePage, IPageObject
    {
        ...
        {
            WriteLogOnMethodEntry(modelName,serialNumber,windowHandle,installerDriver);
        ...
        {
            WriteLogOnMethodEntry(row,findElementTimeout,driver);
        ...
        {
            WriteLogOnMethodEntry(findElementTimeout,serialNumber,windowHandle);
        ...
        {
            WriteLogOnMethodEntry(swapOldDeviceSerialNumber, swapOldDeviceMonoPrintCount, swapOldDeviceColorPrintCount, swapNewDeviceSerialNumber, swapNewDeviceMonoPrintCount, swapNewDeviceColorPrintCount);
            int findElementTimeout = RuntimeSettings.DefaultFindElementTimeout;
        ...
        {
            WriteLogOnMethodEntry(row, findElementTimeout);
        ...
        {
            WriteLogOnMethodEntry(element,elementToVerify,retryCount,findElementTimeout);
            var isConnected = false;
            var retries = 0;
        ...
        {
            WriteLogOnMethodEntry(serial,device);
            if (Method() == "Email") return;
        ...
        {
            WriteLogOnMethodEntry(serial,device,number);
            if (Method() == "Email") return;
            CreateNewSerialNumber(serial);
        ...
        {
            WriteLogOnMethodEntry(serialNumber,serialNumber1,serialNumber2,serialNumber3);
            ClosePopUpModal();
        ...
        {
            WriteLogOnMethodEntry(device,serial);
            try
            {
        ...
        {
            WriteLogOnMethodEntry(device,serial,number);
            try
            {
        ...
        {
            WriteLogOnMethodEntry(model,serial,totalPrint,colourPrint,monoPrint,emailAddress,subject);
...
//                CompleteCloudInstallationComfirmationElement.Click();
        ...
        {
            WriteLogOnMethodEntry(number1,number2,number3,number4);
            if (Method() == "Email")
            {
                TestCheck.AssertIsEqual(true, CompleteInstallationComfirmationElement.Displayed,
                "Installation not successful");
