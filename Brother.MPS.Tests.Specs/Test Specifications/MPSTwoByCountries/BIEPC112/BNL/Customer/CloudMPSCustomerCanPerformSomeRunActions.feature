...
Feature: DutchCustomerCanPerformSomeRunActions
	In order for customer to perform some run portion actions
	As a customer
	I want to be to sign in using my newly generated credentials


Scenario Outline: MPS Dutch Customer cannot order consumable before initial communication with device
...
Scenarios:
	...
	| Cloud MPS Local Office Approver | Netherlands | Purchase & Click with Service | Minimum Volume | Cloud MPS Dealer | Email  | Cloud MPS Customer |



Scenario Outline: MPS Dutch Customer can order consumable after initial communication with device
...
Scenarios:
	...
	| Role               | Country     |
	| Cloud MPS Customer | Netherlands |
